-- Business-wide collaboration and member management.
-- Run this file once in the Supabase SQL Editor after migrations 001 and 002.
...
begin
  ...
  return query
  select bm.user_id,
         coalesce(nullif(p.display_name, ''), nullif(u.email, ''), 'Costudio member'),
         coalesce(u.email, ''),
         bm.role,
         bm.created_at
...
begin
  ...
  values (target_business_id, invited_user_id, member_role)
  on conflict (business_id, user_id) do update set role = excluded.role;

  return query
  select invited_user_id,
         coalesce(nullif(p.display_name, ''), member_email),
         coalesce(u.email, ''),
         bm.role
  from public.business_members bm
  join auth.users u on u.id = bm.user_id
...
create or replace function public.remove_business_member(target_business_id uuid, target_user_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  if not exists (
    ...
    raise exception 'Only workspace owners and admins can remove members.' using errcode = '42501';
  end if;
